...
| 엔드포인트 | 반환 |
|---|---|
| `GET /api/page?market=&product=` | 제품 페이지 1벌 — 히어로 · 1층 스펙 · 요약(제품 축 + 마켓 축 병합) · 커뮤니티 리뷰 |
...
- **Important:** `/api/page` 는 **저장된 요약만** 읽는다(`with_summary=False`). 페이지 로드마다
  요약을 생성하면 열 때마다 과금된다 — 생성은 `pipeline.generate_summaries`(제품 축)와
  `pipeline.generate_market_summaries`(주문 축) 로 따로 돈다.
- **Important:** 응답의 기준 한 칸은 **두 payload 중 그 기준의 축이 소유한 쪽**에서 나온다
  (`_pick`, [ADR-0015](../docs/adr/0015-market-scope-order-criteria.md)). 고객 응대·배송은 마켓 행
  (`product IS NULL`), 나머지 넷은 제품 행. 줄마다 `scope` 를 실어 보내니 **프런트가 키 이름으로
  다시 판단하지 말 것** — 규칙이 두 벌이 된다.
