...
- **Important:** 링크·임베드 **판단은 UI 가 하지 않는다** — `slime_rag.source_links` 의 순수 함수
  (`permalink`/`embed_url`/`group_evidence_sources`)가 정하고 여기선 그리기만 한다(ADR-0009).
- **Don't:** 임베드 아래 텍스트 링크를 지우지 말 것. 삭제·비공개 게시물이면 프레임 안에 인스타
  에러 화면이 뜨고 브라우저가 서드파티 프레임을 막으면 빈 칸이 되는데 **서버에서 감지 불가** —
  링크가 유일한 폴백이다. 프라이버시(제3자 쿠키)·건수 단위·evidence≠인용 캡션도 같은 이유로 유지.
- **Note:** 미디어 임베드는 **판매자 본인 게시물(1층 스펙)만**. 유저 후기 미디어는 임베드하지 않는다.
- **Note:** 사이드바 1층 패널의 마켓 로고도 같은 규칙 — `pipeline.market_logo()` 결과를 그리기만 하고
  분기 판단은 `source_links.logo_asset` 이 한다([ADR-0010](../docs/adr/0010-market-logo-assets.md)).
  **`@handle` 링크는 로고 종류와 무관하게 항상 그린다** — 링크백이 재호스팅 예외의 조건이라서,
  지우면 정책 위반이지 스타일 변경이 아니다.
