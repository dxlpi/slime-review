...
| `test_product_repair.py` | 제품명 귀속 복구 — 유령 제거(풀조합·향료·광역태그)와 **진짜 제품 보존**(해시태그면 1층 부재와 무관 유지 · 같은 글의 별개 제품은 개명도 삭제도 안 함)의 균형 + 접기 계약(이중 계상 방지·생존자 선택·보류 미접기) + **③′ 레지스트리 2단 타이브레이크**(1층이 0건일 때만 개입 · **1층 판정을 못 뒤집는다**=합집합 금지 회귀 · 다중 일치는 보류 · 앞 규칙 미부활). 개발 중 **양방향으로 한 번씩 깨진** 자리라 회귀 게이트로 둔다 (14 케이스) |
| `test_market_prefix.py` | 제품명에 섞인 **마켓 접두** 분리(`linking.split_market_prefix`→`link`) — 자모 접두 분리(공백 유무 무관) · 맨몸 표기는 제품 보류 · **완성형 초성 환원 금지**(`포도`≠푸딩 회귀) · 접두→마켓 승격 + **전용 확신도**(롤백 열쇠) · 명시 마켓 우선 · 충돌 접두는 마켓 보류하되 제품명은 분리 · 충돌 접두는 역인덱스도 미발동 · 항목 마켓(`mentioned_market`) 우선순위 3단 · 약칭은 **마켓 스코프**(전역 적용 금지) (17 케이스) |
| `test_market_backfill.py` | 마켓 백필/복구 **provenance** 계약 — 백필 확신도가 모든 경로와 값이 안 겹침(층별 롤백) · 눈금 배치 · 채우면 확신도 동시 기록 · 마켓 변경 시 evidence·tokens·embedding 동시 재생성 · 확신도 백필은 마켓 무변경·엇갈림 미승인 · 전 복구 함수 `dry_run` 기본·키워드 전용 · LLM 0회 · **되돌린 본문 스캔 가드 미부활** + 디시 제자리 복구(`dc_attribution_target` 순서 회귀 · 접기 키=DB 제약 키 · 접기 외 삭제 없음) (14 케이스) |
| `test_alias_candidates.py` | 약칭 **후보** 유도 — prefix/edit-1/registry 3종 · 편집거리 2는 미배출 · 캡션 본문 미유출(ADR-0013) · `dry_run` 기본 무쓰기 · **`data/product_aliases.json` 자동 병합 금지**(쿨라임 사고 회귀) · LLM 0회 (12 케이스) |
| `test_layer1_collection.py` | 1층 수집 **누적성** 계약 — `_upsert_spec` 전 값 칸 COALESCE 보호(+`beads` 는 cardinality) · `ingest_seller_profiles` 기본 대상이 KB 전체(스펙 보유 마켓 포함) · `only_missing` 은 명시 옵션 · `dry_run` 무접촉. 액터가 최신 ~12글만 주므로 **수집은 누적이지 교체가 아니다** + **중간 커밋 계약**(유료 N건마다 커밋 · 중단 시 기지불분 보존 · 주기 단위는 순회 위치가 아니라 **값이 나간 건수**) + **예상=실제 계약**(`dry_run` 의 `paid_posts` 가 유료 런의 콜 수와 정확히 일치 · dry 는 쓰기 0) (9 케이스) |
...
| `test_spec_overrides.py` | 1층 스펙 사람 검수 오버레이([ADR-0016](../docs/adr/0016-human-in-the-loop-spec-review.md)) — **오버레이가 non-null LLM 값을 이긴다**(D3 회귀 가드: 안 걸리면 재수집이 검수를 조용히 되돌린다) · `apply`/`record` 순수성(배열 별칭 포함) · `beads`·permalink 는 큐 밖 · `unknown` 은 큐에서만 빼고 **값을 안 만든다** · `was` 왕복 보존 · 원자적 저장 · 고아 노출 · `OVERRIDABLE` 밖 칸 거부 (8 케이스, 정본 파일 무접촉) |
...
python -m eval.test_spec_overrides   # 1층 스펙 사람 검수 오버레이(오버레이 우선·unknown·원자성)
