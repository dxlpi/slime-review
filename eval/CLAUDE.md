...
| `test_layer1_collection.py` | 1층 수집 **누적성** 계약 — `_upsert_spec` 전 값 칸 COALESCE 보호(+`beads` 는 cardinality) · `ingest_seller_profiles` 기본 대상이 KB 전체(스펙 보유 마켓 포함) · `only_missing` 은 명시 옵션 · `dry_run` 무접촉. 액터가 최신 ~12글만 주므로 **수집은 누적이지 교체가 아니다** + **중간 커밋 계약**(유료 N건마다 커밋 · 중단 시 기지불분 보존 · 주기 단위는 순회 위치가 아니라 **값이 나간 건수**) (8 케이스) |
