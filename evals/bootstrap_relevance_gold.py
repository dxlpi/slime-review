...
- 항목: {text, platform, type, collected_for{market,slime}, label{keep, M, Q, E, why, draft:true}}
...
def draft_label_dc(text: str) -> dict:
    """초안 라벨 — 절대 축은 M/Q/E 이진 3축(배타 4분류 `kind` 는 폐기, resale 범주도 폐기:
    아모스 갤은 비거래 도메인). 축은 서로 배타가 아니므로 독립적으로 켠다."""
    t = text.strip()
    q = int(bool(Q_RE.search(t)))
    if NEWS_RE.search(t):
        return {"keep": False, "M": 1, "Q": 0, "E": 0, "why": "news-bleed(디시 실시간 위젯)", "draft": True}
    if META_RE.search(t):
        return {"keep": False, "M": 1, "Q": q, "E": 0, "why": "갤 운영/메타 잡담", "draft": True}
    if PROD_RE.search(t):
        return {"keep": True, "M": 0, "Q": q, "E": int(not q), "why": "제품/후기 어휘", "draft": True}
    # 제품 신호 없고 짧은 잡담
    return {"keep": False, "M": 0, "Q": q, "E": 0, "why": "제품 무관 잡담(추정)", "draft": True}
...
def load_ig_items() -> list[dict]:
    ...
    for p in posts:
        ...
        # 3축은 인스타에서 런타임 미사용이지만 스키마상 필수(AC2) — 초안은 전부 0, 사용자 교정 대상.
        out.append({"text": cap[:400], "platform": "instagram", "type": "post",
                    "collected_for": {"market": "머머", "slime": slime},
                    "label": {"keep": True, "M": 0, "Q": 0, "E": 0,
                              "why": "실 슬라임 캡션(온토픽)", "draft": True}})
    return out
...
def boundary_pairs(dc_items: list[dict], ig_items: list[dict]) -> list[dict]:
    ...
    for x in keeps_dc:
        pairs.append({"text": x["text"], "platform": "dcinside", "type": x["type"],
                      ...
                      "label": {"keep": False, **{k: x["label"][k] for k in ("M", "Q", "E")},
...
def main():
    ...
    OUT.write_text(json.dumps({
        "_doc": ("관련성 골드셋(US-006). 실텍스트만(D5). label.draft=true 는 자동 초안 — 사용자 교정 필요. "
                 "collected_for=수집 타깃(앵커). 절대 축=M(메타)/Q(질문)/E(1인칭 실사용 평가) 이진 3축. "
