...
```
폼볼, 촉감류(점토), 디폼, 난사, 눈꽃, 지글리, 크런치, 빈백, 클라우드, 샤베트, 클리어, 버글리, 젤라또, 빨대, 라이스볼
```
...
무시하는 건 '구매자 평가'다 — 판매자가 자기 제품을 직접 서술한 질감 설명은 공식 정보이므로
official_texture 로 1~2문장 요약해 뽑는다(캡션 어휘 그대로 압축만, 미화 금지).
official_texture 는 '~해요'체로 끝낸다 — 바꾸는 건 종결어미뿐이고 촉감 어휘는 캡션 그대로다.
...
[{
  ...
  "official_texture": "초반엔 탱글하다가 기포가 차면 몽글해지고, 밀도가 높아 묵직한 편이에요.",
                                           // 판매자가 쓴 '만지면 어떤가' 서술 1~2문장. 언급 없으면 null
