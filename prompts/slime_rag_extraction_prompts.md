...
```
폼볼, 촉감류(점토), 디폼, 난사, 눈꽃, 지글리, 크런치, 빈백, 클라우드, 샤베트, 클리어, 버글리, 젤라또, 빨대, 라이스볼, 수수깡, 자바칩, 스티로폼, 납작블럭
```
두 갈래다 — **내장물형**(폼볼·디폼·빈백·빨대·라이스볼·수수깡·자바칩·난사·스티로폼·납작블럭)과
**베이스 성격어**(촉감류(점토)·클리어·크런치·버글리·지글리·샤베트·젤라또·클라우드·눈꽃).
내장물이 우선이되 성격어도 성립하면 둘 다 적는다(성격어를 앞에) — 예) `["크런치","디폼"]`.
`납작블럭`은 표기가 갈린다(`납작블록`) — 통제어휘 표기 하나로 접는다. 네모블럭·샌드블럭 등
**다른 블럭은 종류가 아니다**(`beads` 행).
...
캡션 첫머리(해시태그 줄 직후, 본문 문단 전)의 짧은 나열 줄이 풀조합의 정본이다. 본문 산문의
'베이스는 ~' 언급은 다른 제품과의 비교이거나 질감 서술이므로 glue_composition 으로 쓰지 마라.
머리말 줄이 둘이면 풀조합·향 순서는 고정이 아니니 내용으로 판별한다. 그 줄의 재료는 하나도
빠뜨리지 말고 전부 옮기되, 괄호 주석('(크런키 베이스)')은 재료가 아니므로 제외한다.
