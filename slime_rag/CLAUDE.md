...
- **Important:** 스레드 market 상속은 **채우기 전용**이다(2026-08-10). 자기 마켓을 뽑은 조각은
  ...
  **Important:** 그리고 상속의 **출처는 글 본문 하나뿐**이다(2026-08-10). 예전엔 글이 마켓을
  안 밝히면 `next(d.get("market") for d in docs …)` 로 **아무 댓글의 마켓이나** 집어 스레드
  전체에 물려줬다. 댓글의 마켓은 **그 댓글 작성자의 것**이지 스레드의 것이 아니다 — 실측:
  `빈짱` 21행 중 **19행이 한 스레드**에서 나왔고 출처는 댓글 하나의 `ㅂㅉ` 였다. 마켓이 붙은
  스레드가 **95/95(100%) 단일 마켓**이었는데, 원문 대조 결과 이 갤의 지배적 형태는 한 글에
  여러 마켓이 나열되는 추천/첨삭 글이다 — 100% 는 나올 수 없는 값이라 그 자체가 증거였다.
  **Don't:** 그 폴백을 되살리지 말 것. 항목별 마켓이 필요하면 폴백이 아니라
  `reviews[].mentioned_market`(항목 칸)이 자리다. 상속이 실제로 몇 건 걸렸는지는
...
- **Important:** 같은 조각의 제품명 **포함관계는 자동으로 접지 않는다** —
  ...
  `미니빠코볼` ⊃ `빠코볼` 같은 **접미** 포함을 원리적으로 못 잡는다. 여긴 한 조각이 같은
  문장을 두 입도로 내보낸 흔적이라 범위를 조각으로 좁히고 방향을 전부 본다.
  **Don't:** 포함관계만으로 병합하지 말 것 — 같은 조각의 `빠코볼`/`미니빠코볼` 은 **진짜 다른
  제품**이다(202004 실측). `keep_distinct` 가드가 그 흉터고, **과잉 병합의 손실은 화면에 안
  보인다**(후기 두 건이 한 건이 될 뿐 빈칸이 안 생긴다). 승격은 사람이
  `data/product_aliases.json` 에서 한다.
  **Note:** 실측 5쌍 → 마켓 접두 분리 뒤 **3쌍**(`ㅁㅁㄴ 새튀반`·`ㅇㅉ 쿠키슈크림번` 이 접혔다) —
  ②를 고치면 ③이 대부분 자동 해소된다는 계획의 예측이 값으로 확인된 자리다.
- **Important:** 종류어·재료어·조각난 이름도 제품이 아니다 — `extract.is_non_product_word`
  ...
  **Important:** 이 게이트는 **마켓 접두를 뗀 뒤에만** 돈다. 두 규칙은 개별적으로는 둘 다
  맞는데 **합성 순서가 틀리면 조용히 반대로 동작한다** — 실측으로 양방향 다 재현됐다:
  ① 단어 게이트가 먼저면 `ㅇㅊ` 가 '자모뿐인 이름'으로 비워져 **마켓 신호가 사라지고**, 그 행은
  스레드 도장 마켓을 0.95(직접 매칭)로 물려받는다 — 되돌릴 표식 없는 오귀속이라 NULL 보다 나쁘다.
  ② 반대로 `ㅂㅇㅍ 빨대` 의 나머지 `빨대`(종류어)는 게이트를 이미 지나쳐 제품으로 남는다.
  **Don't:** 그래서 `extract_thread` 는 이 게이트를 **부르지 않는다** — 추출 층엔 KB 가 없어
  접두를 뗄 수 없다. 합성은 KB 를 아는 두 곳에서만 이 순서로 돈다: `linking.link`(수집 경로) ·
  `pipeline.dc_attribution_target`(적재분 복구).
  게이트: `eval/test_market_prefix.py::test_extract_to_link_composition_*` — `link` 와
  `split_market_prefix` 를 **따로** 보는 테스트는 이 결함이 살아 있어도 전부 통과한다.
- **Important:** 보류(제품명 None) 항목은 **내용이 완전히 같을 때만** 접는다(`_held_fingerprint`).
  ...
  **Note:** `_fold_by_product` 의 호출처는 `repair_product_names` **하나뿐**이고 그건 해시태그가
  없으면 즉시 반환한다 — 즉 이 접기는 **인스타 경로에서만** 돈다. 디시의 말더듬은 수집 시점이
  아니라 `pipeline.repair_dc_attribution` 의 `held` 접기가 치운다. 그래서 새 디시 말더듬은
  복구를 다시 돌릴 때까지 쌓인다(현재 감사값 0). 두 소스가 같은 자리에서 접힌다고 가정하지 말 것.
