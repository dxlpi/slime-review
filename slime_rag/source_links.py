...
표시 계층은 표시 전용이어야 한다. DB·네트워크·UI 프레임워크를 일절 import 하지 않는
...
EVIDENCE_NOT_QUOTE_CAPTION = (
    "근거 문구는 원문 인용이 아니라 추출된 항목의 구조화 요약이에요 — 원문은 링크로 확인해 주세요."
)

# 후기 건수(행수)와 근거 목록 건수가 다른 두 가지 이유를 **둘 다** 밝힌다.
# 하나만 쓰면 오도한다: 팬아웃만 쓰면 댓글 묶임이 숨고, 묶임만 쓰면 팬아웃이 숨는다.
EVIDENCE_COUNT_CAPTION = (
    "후기 건수는 제품별 행 수라 한 글이 여러 건으로 세어질 수 있고, 근거 목록은 원문 조각 수예요. "
    "디시는 댓글 앵커가 없어 같은 스레드의 글·댓글이 한 줄로 묶여요."
)
...
EMBED_PRIVACY_CAPTION = (
    ...
    "프레임을 차단하면 빈 칸으로 보일 수 있어요(아래 링크로 원문 확인)."
)
...
# 여기 있는 건 **표시 정책**뿐이다. 수집·다운로드는 `slime_rag.logos`, 렌더는 프런트엔드.
...
def logo_asset(market_entry: dict | None, *, exists=Path.exists) -> dict:
    ...
    렌더는 프로세스 CWD 에 의존하면 안 된다 — 서버를 리포 밖에서 실행하는
...
# ---------------------------------------------------------------- 본문 발췌(표시 정책, ADR-0013 §3)
# 화면에 내려보낼 본문 길이. **이 상수가 ADR-0013 §5 의 '공개 전환 스위치'다** — 공개 배포로
# 바꿀 때 여기 한 곳만 줄이면(또는 0 으로 두어 스니펫만 쓰면) 화면 전체가 따라온다.
# 흩어놓으면 공개 시점에 반드시 하나를 빠뜨린다.
EXCERPT_CHARS = 300          # 비공개 데모 기준
EXCERPT_CHARS_PUBLIC = 120   # 참고값: 공개 전환 시 권장(발췌+링크로 인용 범위 유지)

_SENT_END = ("。", ".", "!", "?", "…", "\n")


def excerpt(body: str | None, limit: int = EXCERPT_CHARS) -> str | None:
    """원문 본문 → **서버에서 자른** 발췌. 없으면 None.

    ⚠️ 이 함수가 나가는 지점이 곧 브라우저가 받는 전부다. CSS `line-clamp` 로 접는 건 발췌가
       아니다 — 전문이 이미 클라이언트에 도달한 상태라 표시 제한이 성립하지 않는다(ADR-0013 §3).

    자르는 자리는 문장 경계 우선, 없으면 어절 경계다. 글자 수로 뚝 자르면 단어 중간이 끊겨
    읽는 사람이 '깨진 데이터'로 읽는다. 뒤에 `…` 를 붙여 잘렸음을 명시한다.
    """
    if not body:
        return None
    text = " ".join(str(body).split())          # 개행·연속공백 정리(카드 높이 안정)
    if len(text) <= limit:
        return text
    head = text[:limit]
    cut = max(head.rfind(c) for c in _SENT_END)  # 문장 경계
    if cut < limit * 0.5:                        # 너무 앞에서 끊기면 어절 경계로
        cut = head.rfind(" ")
    if cut <= 0:                                 # 공백 없는 긴 문자열(해시태그 덩어리 등)
        cut = limit
